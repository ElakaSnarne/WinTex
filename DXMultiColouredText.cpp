...
void CDXMultiColouredText::SetTextPD(LPCWSTR text, Rect rect)
{
	if (_vertexBuffer != NULL)
	{
		_vertexBuffer->Release();
	...
	if (_printableCharacters > 0)
	{
		...
		if (pVB != NULL)
		{
			...
			while (pWL != NULL)
			{
				...
				while (pWL != NULL && pWL->Text() != NULL && (pixelsLeft - (pWL->Pixels() + pWidths[0])) > -0.01)	// Enough room for the next word?
				{
					wordsInLine++;
					pixelsLeft -= pWL->Pixels() + pWidths[0];
					pWL = (CMCWordList*)pWL->Next();
...
void CDXMultiColouredText::SetTextPD2(LPCWSTR text, Rect rect)
{
	if (_vertexBuffer != NULL)
	{
		_vertexBuffer->Release();
	...
	while (TRUE)
	{
		...
		if (c == 0 || c == 0x20 || c == 0xa)
		{
			...
			// New word, possibly force new line
		...
		else if (c > 0x20 && c <= 0x7f)
		{
			...
			mc.Pixels = ((float)pWidths[c - 0x20]) * pConfig->FontScale;
	...
	if (_printableCharacters > 0)
	{
		...
		if (pVB != NULL)
		{
			...
			for (auto pW : words)
			{
				if (pW == NULL || pW->Pixels > pixelsLeft)
				{
					sx = rect.Left;
					sy -= fh;
					_lines++;
