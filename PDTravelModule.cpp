...
void CPDTravelModule::BeginAction()
{
	...
	for (auto location : _subLocations)
	{
		...
		if (pPDSL->ParentLocation == _area && CGameController::GetData(PD_SAVE_TRAVEL + pPDSL->Id) != 0 && pPDSL->HitTest(x, y))
		{
			...
			else
			{
				...
				for (auto subLocation : _subLocations)
				{
					...
					if (pSPDSL->IndicatorX == 0 && pSPDSL->ParentLocation == pPDSL->Id && CGameController::GetData(PD_SAVE_TRAVEL + pSPDSL->Id) != 0)
					{
...
void CPDTravelModule::RenderLocations()
{
	for (auto location : _subLocations)
	{
		...
		{
			if (CGameController::GetData(PD_SAVE_TRAVEL + pPDSL->Id) != 0)
			{
				std::unordered_map<int, int> colourMap;
				colourMap[2] = pPDSL->ColourIndex;
